...
export default function BookingsPage() {
  ...
  const validate = (includeContact = true) => {
    ...
    else if (includeContact && selectedQuote.covered && Number(form.amountPaid) > selectedQuote.total) errors.amountPaid = `Amount paid cannot exceed the total of â‚±${selectedQuote.total.toLocaleString()}.`;
  ...
  const save = async () => {
    ...
    else if (form.paymentStatus === BookingStatus.Reserved && Number(form.amountPaid) > addGrandTotal) errors.amountPaid = `Reservation amount cannot exceed the total of â‚±${addGrandTotal.toLocaleString()}.`;
  ...
  return <div className="space-y-6 max-w-[1600px] w-full mx-auto px-4 sm:px-6 pb-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
    ...
    <div className="grid gap-4 sm:grid-cols-3"><Metric label="Matching bookings" value={filteredBookings.length.toString()} /><Metric label="Paid" value={filteredBookings.filter(b => b.status === 'Paid').length.toString()} /><Metric label="Remaining balance" value={`â‚±${totalRemaining.toLocaleString()}`} /></div>
      ...
      {paginatedBookings.map(b => (
        <div key={b.id} className="rounded-xl border dark:border-white/10 p-4 space-y-3">
          ...
          <div className="text-sm text-muted-foreground flex justify-between">
            <div>
              <div>{b.courtName} Â· {format(new Date(`${b.bookingDate}T00:00:00`), 'MMM d')}</div>
              <div className="text-xs">{time(b.startTime)}â€“{time(b.endTime)}</div>
            </div>
            <div className="text-right">
              <div className="font-medium text-foreground">â‚±{b.amountPaid.toLocaleString()} / â‚±{b.totalAmount.toLocaleString()}</div>
          ...
          <div className="flex flex-wrap gap-2 pt-2 border-t">
            ...
            {(b.status === 'Paid' || b.status === 'Reserved') && b.paddleRentalQuantity < 50 && <Button variant="outline" size="sm" className="flex-1 min-w-[30%]" onClick={() => { setPaddleRentalTarget(b); setPaddleRentalQuantity(1); }}><PaddleIcon className="mr-1.5 h-3.5 w-3.5" forceLight /> Paddle</Button>}
          ...
          <div className="space-y-5">
            ...
            <section className="grid gap-4 rounded-2xl border border-slate-200 bg-slate-50/60 p-4 shadow-sm sm:grid-cols-2 sm:p-5 dark:border-white/10 dark:bg-[#323234]">
                ...
                <Select value={form.promoId?.toString() || 'none'} onValueChange={value => setForm({...form, promoId: value !== 'none' ? Number(value) : null})}>
                  ...
                  <SelectContent>
                    ...
                    {promos.filter((p) => isPromoAvailable(p, form.rateType)).map((p) => (
                      <SelectItem key={p.id} value={p.id.toString()}>{p.code} - {p.type === 'Percentage' ? `${p.value}%` : `â‚±${p.value}`} off</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
              ...
              <div className="sm:col-span-2">
                <label className="text-[11px] font-bold text-slate-700 uppercase tracking-wider dark:text-slate-200">flex items-center gap-1.5"><PaddleIcon className="w-4 h-4" /> Paddle Rental</label>
                <div className="mt-1.5 flex items-center justify-between rounded-xl border bg-white p-3 dark:bg-[#3a3a3c]">
                  <div>
                    <p className="text-sm font-semibold flex items-center gap-1.5"><PaddleIcon className="w-4 h-4" /> Selkirk Paddle â€“ â‚±100 each</p>
                ...
                {form.paddleRentalQuantity > 0 && <p className="mt-2 text-right text-sm font-semibold text-primary">Paddle rental: â‚±{paddleRentalFee.toLocaleString()}</p>}
              </div>
    ...
    <div className="space-y-4 text-sm mt-2">
      <div className="rounded-xl border dark:border-white/10 bg-white dark:bg-[#2c2c2e] overflow-hidden shadow-sm">
        ...
        <div className="p-4 grid gap-3">
          ...
          <Detail k="Email" v={b.email || 'â€”'} />
          <Detail k="Phone" v={b.phone || 'â€”'} />
        </div>
      </div>
      ...
      <div className="rounded-xl border dark:border-white/10 bg-white dark:bg-[#2c2c2e] overflow-hidden shadow-sm">
        ...
        <div className="p-4 grid gap-3">
          ...
          <Detail k="Schedule" v={`${format(new Date(`${b.bookingDate}T00:00:00`), 'MMMM d, yyyy')} Â· ${time(b.startTime)}â€“${time(b.endTime)}`} />
      ...
      <div className="rounded-xl border dark:border-white/10 bg-white dark:bg-[#2c2c2e] overflow-hidden shadow-sm">
        ...
        <div className="p-4 grid gap-3">
          <Detail k="Court Booking" v={`â‚±${b.subtotal.toLocaleString()}`} />
          {b.discountAmount > 0 && <Detail k="Discount" v={`-â‚±${b.discountAmount.toLocaleString()}`} valueClass="text-emerald-600 dark:text-emerald-400 font-bold" />}
          {b.paddleRentalQuantity > 0 && <Detail k={`Selkirk Paddle Rental Ã— ${b.paddleRentalQuantity}`} v={`â‚±${b.paddleRentalFee.toLocaleString()}`} />}
          <div className="border-t dark:border-white/10 pt-3 mt-1"><Detail k="Total" v={`â‚±${b.totalAmount.toLocaleString()}`} valueClass="text-lg font-bold" /></div>
          
          <div className="border-t dark:border-white/10 pt-3 mt-1 space-y-3">
            <Detail 
              k="Amount Paid" 
              v={`â‚±${b.amountPaid.toLocaleString()}`} 
              valueClass="text-emerald-600 dark:text-emerald-400 font-semibold"
            />
            {!isCancelled && (
              <Detail 
                k="Balance" 
                v={b.remainingBalance > 0 ? `â‚±${b.remainingBalance.toLocaleString()}` : 'Fully Paid'} 
...
function BalanceStatus({ booking }: { booking: Booking }) { return booking.status === 'Cancelled' ? <div className="text-xs font-medium text-slate-500">Cancelled Â· no remaining balance</div> : <div className={booking.remainingBalance ? 'text-xs text-amber-600' : 'text-xs text-emerald-600'}>{booking.remainingBalance ? `â‚±${booking.remainingBalance.toLocaleString()} remaining` : 'Fully paid'}</div>; }
...
function Detail({ k, v, valueClass }: { k: React.ReactNode; v: React.ReactNode; valueClass?: string }) { return <div className="flex justify-between gap-4 items-center"><span className="text-slate-500 dark:text-slate-400">{k}</span><span className={cn("text-right font-medium text-slate-900 dark:text-slate-100", valueClass)}>{v}</span></div>; }
...
function BookingFields({ form, setForm, courts, rates = [], promos = [], includeContact = false, errors = {}, setErrors, availability, availabilityLoading, currentBooking, now = Date.now() }: any) {
  ...
  return <div className="grid gap-4 py-2">
    ...
    {!!form.startTime && !!form.endTime && <div><div className={cn('rounded-xl border p-3.5', quote.covered ? 'border-primary/20 bg-primary/5' : 'border-amber-200 bg-amber-50 dark:border-amber-800 dark:bg-amber-950/20')}><div className="flex items-center justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Calculated total</p><p className="mt-1 text-xs text-muted-foreground">{quote.covered ? quote.lines.map(line => `${Number.isInteger(line.hours) ? line.hours : line.hours.toFixed(2)} hr Ã— â‚±${line.pricePerHour.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} (${line.pricingId})`).join(' + ') : `No ${form.rateType || RateType.Booking} rate covers the complete time range.`}</p></div><div className="text-right"><p className="shrink-0 text-base font-bold text-primary">{quote.covered ? (discount > 0 ? <><span className="line-through text-muted-foreground font-normal text-sm mr-2">â‚±{quote.total.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>â‚±{finalTotal.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</> : `â‚±${quote.total.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`) : 'â€”'}</p></div></div></div><FieldError message={errors.rate} /></div>}
    {includeContact && <>
      <div>
        <Label>Promo Code (Optional)</Label>
